...
$start = Get-Date
$VerticalScroll = 0
Write-Verbose "Scrolling the WebPage : $URL , to auto-populate all profiles for next 90 Secs" 
# 90 Secs to Infinitely scroll webpage, So that all items are populated that only come when you scroll down
While((Get-Date) -lt $($start + [timespan]::new(0,0,90)))
        {
...
[pscustomobject][ordered]@{        
    ...
    DisplayName = $HTML.all.tags('a') | ?{$_.classname -eq "fullname ProfileNameTruncated-link u-textInheritColor js-nav"} | % innertext  
    Twitterhandle = "@$($HTML.all.tags('b') |  ?{$_.classname -eq 'u-linkComplex-target'} | % innertext)"    
...
Get-TwitterProfile -URL "https://twitter.com/following" -Verbose -OutVariable Result
# saving images from twitter to your local drive
#iwr $UserProfiles[0].ImageURL -OutFile ".\ProfilePictures\$($UserProfiles[0].DisplayName.trim()).png" -Verbose


$Result.Twitterhandle | Out-File -FilePath "C:\Temp\folksIfollow.txt" -Append
